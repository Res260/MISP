...
?>
		...
		<td class="short"><?php echo h($item['Sighting']['attribute_id']);?></td>
		  ...
		  ?>
			<span class="glyphicon glyphicon-trash useCursorPointer" title="Delete sighting" role="button" tabindex="0" aria-label="<?php echo __('Delete sighting');?>" onClick="quickDeleteSighting('<?php echo h($item['Sighting']['id']); ?>', '<?php echo h($rawId); ?>', '<?php echo h($context); ?>');"></span>
		  <?php
			endif;
				?>
			</td>
