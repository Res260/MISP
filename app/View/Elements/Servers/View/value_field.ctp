<?php
  ...
  if ('attachment' == $object['type'] || 'malware-sample' == $object['type'] ) {
    ...
    echo '<span class="hex-value" title="' . __('Hexadecimal representation') . '">' . nl2br(h($sigDisplay)) . '</span>&nbsp;<span role="button" tabindex="0" aria-label="' . __('Switch to binary representation') . '" class="glyphicon glyphicon-repeat hex-value-convert useCursorPointer" title="' . __('Switch to binary representation') . '"></span>';
  } else {
    $sigDisplay = str_replace("\r", '', $sigDisplay);
    echo nl2br(h($sigDisplay));
  }
  if (isset($object['validationIssue'])) echo ' <span class="glyphicon glyphicon-warning-sign" title="' . '__('Warning, this doesn\'t seem to be a legitimate ') . strtoupper(h($object['type'])) . __(' value') . '">&nbsp;</span>';
?>
